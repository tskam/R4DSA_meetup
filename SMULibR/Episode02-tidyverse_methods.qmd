...
The code chunk below will check if tidyverse and psych packages have been installed,

-   if Yes then load onto R environment,
-   if No then install these to packages then only load them onto R environment.
...
renderthis::to_pdf(from = "R_series_Episode02-tidyverse_methods.qmd")
```
